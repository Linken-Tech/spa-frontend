...
function UpdateVehicles() {
  ...
  if (
    location.state.PriceOfSale !== null &&
    location.state.PriceOfSale !== "" &&
    location.state.PriceOfSale !== "0" &&
    location.state.PriceOfSale !== "0.00"
  ) {
    saleRental = "Sale";
  } else {
    saleRental = "Rental";
  ...
  const [vehicleDocumentList, setVehicleDocumentList] = useState(
    location.state.VehicleDocuments
  );
  ...
  useEffect(() => {
    const newImageUrls = [];
    newVehicleImages.forEach((image) =>
  ...
  function onImagePreviewClick(imageId) {
    if (window.confirm("Confirm to delete this image?")) {
      setVehicleImageList(
        vehicleImageList.filter((item) => item.id !== imageId)
      );
  ...
  function onDocumentPreviewClick(docId) {
    if (window.confirm("Confirm to delete this document?")) {
      setVehicleDocumentList(
        vehicleDocumentList.filter((item) => item.id !== docId)
      );
  ...
  const saveVehicle = (event) => {
    ...
    if (window.confirm("Confirm to save?")) {
      ...
      newVehicleDocuments.forEach((file) =>
        fileData.append("vehicle_documents", file, file.name)
      );
      deletedVehicleImages.forEach((data) =>
        fileData.append("delete_images", data)
      );
      deletedVehicleDocuments.forEach((data) =>
        fileData.append("delete_documents", data)
      );
            ...
            {vehicleImageList.map((item, index) => (
              <Col key={index} lg="auto" sm="auto" md="auto" className="mt-2 ">
                <div className="image-column">
                  ...
                  <button
                    type="button"
                    className="text-white delete-icon-middle"
                    onClick={() => onImagePreviewClick(item.id)}
                  >
                    <RiDeleteBin6Line />
                  </button>
                </div>
            ...
            {accessoriesData.map((option, index) => (
              ...
              >
                <input
                  ...
                  type="checkbox"
                  defaultChecked={
                    accessoriesList.includes(option.value) ? true : false
                  }
                  key={index}
                  id={option.value}
                  value={option.value}
